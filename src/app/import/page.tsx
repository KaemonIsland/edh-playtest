...
export default function ImportPage() {
  ...
  const [savedDecks, setSavedDecks] = useState<ShowcaseDeckMeta[] | null>(null);
  useEffect(() => {
    void getRepo().listDecks().then(setSavedDecks);
  }, []);

  const playSavedDeck = async (deckId: string) => {
    const full = await getRepo().getDeck(deckId);
    if (!full) return;
    saveCurrentDeck(full.deck);
    saveBotDecks([]); // solo by default; add opponents in-game
    loadDeck(full.deck);
              ...
              <div className="rounded-lg border border-stone-800 bg-stone-950 p-3">
                ...
                <div className="flex max-h-56 flex-col gap-1 overflow-y-auto">
                  {savedDecks.map((d) => (
                    <button
                      key={d.id}
                      onClick={() => void playSavedDeck(d.id)}
                      className="group flex items-center gap-3 rounded-md border border-stone-800 bg-stone-900/60 px-2.5 py-2 text-left transition hover:border-emerald-600/60 hover:bg-stone-900"
                    >
                      {d.commanderArt ? (
                        ...
                        <img
                          src={d.commanderArt}
                          alt=""
                          className="h-9 w-14 shrink-0 rounded object-cover object-[center_25%]"
                        />
                      ) : (
                        <div className="h-9 w-14 shrink-0 rounded bg-stone-800" />
                      )}
                      <div className="min-w-0 flex-1">
                        <div className="truncate text-sm font-semibold text-stone-200">{d.name}</div>
                        <div className="truncate text-[11px] text-stone-500">
                          {d.commanderNames.join(" + ") || "No commander"}
                        </div>
                      </div>
                      ...
                      <span className="shrink-0 rounded-md bg-emerald-700 px-3 py-1 text-xs font-bold text-white opacity-0 transition group-hover:opacity-100">
                        ▶ Play
                      </span>
                    </button>
                  ))}
                ...
                <p className="mt-2 text-[10px] text-stone-600">
                  Launches solo — add bot opponents in-game via Playtester actions → Manage
                  opponents. Or paste a fresh list below.
                </p>
              </div>
            )}
