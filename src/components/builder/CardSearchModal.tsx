...
import { Backpack, Crown, Maximize2, Minimize2, X } from "lucide-react";
...
export function CardSearchModal({
  ...
  initialOtag?: string;
  /** Pre-applied filters (e.g. Browse collection: commander identity). */
  initialFilters?: Partial<CardFilters>;
  /** Override the remembered scope (Browse collection forces "collection"). */
  initialScope?: SearchScope;
  /** Run immediately on open, even with an empty query. */
  autoRun?: boolean;
  ...
  const [scope, setScope] = useState<SearchScope>(() => initialScope ?? getSearchScope());
  const ownedOnly = scope === "collection";
  /** Expanded = near-fullscreen, for scanning lots of cards at once. */
  ...
  // Canonical list ordering (lib/cards/sort): color by default, like every
  // other card list in the app. Printings of the same card stay adjacent
  // (newest first) because the comparator ties on identical name/cmc/colors.
  const [sort, setSort] = useState<CardSort>("color");
  const sorted = useMemo(() => {
    if (!results) return null;
    const cmp = cardComparator(sort, (c) => priceOf(c, "nonfoil") ?? 0);
    return [...results].sort(
      (a, b) => cmp(a, b) || (b.released_at ?? "").localeCompare(a.released_at ?? ""),
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [results, sort, priceVersion]);
  ...
  useEffect(() => {
    if (initialQuery.trim() || initialOtag?.trim() || autoRun) void run();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  return (
    <ModalShell onClose={onClose} size={expanded ? "full" : "xl"} anchor="top">
      <div>
        {/* Search bar */}
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="shrink-0 rounded px-2 py-1 text-stone-500 hover:text-stone-200"
            title={expanded ? "Shrink to the default size" : "Expand to fill the screen"}
          >
          ...
          <div className="mt-2 flex flex-col gap-3 rounded-lg border border-stone-800 bg-stone-900/50 p-3">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <ColorPicker
                ...
                size={5}
              />
              <ColorModeSeg value={filters.colorMode} onChange={(colorMode) => setF({ colorMode })} />
            </div>

            {/* Numeric */}
            <div className="grid gap-2 sm:grid-cols-3">
              <NumberFilter inline label="Mana value" op={filters.mvOp} value={filters.mv} onOp={(mvOp) => setF({ mvOp })} onValue={(mv) => setF({ mv })} onSubmit={() => void run()} />
              <NumberFilter inline label="Power" op={filters.powerOp} value={filters.power} onOp={(powerOp) => setF({ powerOp })} onValue={(power) => setF({ power })} onSubmit={() => void run()} />
              <NumberFilter inline label="Toughness" op={filters.toughnessOp} value={filters.toughness} onOp={(toughnessOp) => setF({ toughnessOp })} onValue={(toughness) => setF({ toughness })} onSubmit={() => void run()} />
            </div>
            ...
            <>
              <div className="mb-2 flex items-center gap-2">
                <p className="min-w-0 flex-1 truncate text-[11px] text-stone-500">
                  ...
                  {shownCount < sorted.length ? ` (showing ${shownCount})` : ""} —{" "}
                  {deckMode
                    ? "use “+ Add to deck”, or click a card for full details."
                    : "use −/+ to add a printing, or click a card for full details."}
                </p>
                <SortSelect compact value={sort} onChange={(s) => setSort(s as CardSort)} />
              </div>
              <div
                data-pv={priceVersion}
                className={`${CARD_GRID} ${expanded ? "max-h-[76vh]" : "max-h-[62vh]"} overflow-y-auto`}
              >
                {sorted.slice(0, shownCount).map((card) => (
                  <PrintingTile
                    key={card.id}
                    card={card}
                ...
                {shownCount < sorted.length && (
                  ...
                  >
                    Load more ({sorted.length - shownCount} more)
                  </button>
                )}
              </div>
            </>
