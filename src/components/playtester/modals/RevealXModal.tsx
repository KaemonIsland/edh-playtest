"use client";

import { useMemo, useState } from "react";
import { PLAYER_ID, useGameStore } from "@/lib/game/store";
import { useUiStore } from "@/lib/game/uiStore";
import { CardImage } from "@/components/cards/CardImage";
import { Modal } from "./Modal";

type Dest = "top" | "hand" | "battlefield" | "graveyard" | "exile" | "bottom";

const DESTS: { value: Dest; label: string }[] = [
  { value: "top", label: "Keep on top" },
  { value: "hand", label: "Hand" },
  { value: "battlefield", label: "Battlefield" },
  { value: "graveyard", label: "Graveyard" },
  { value: "exile", label: "Exile" },
  { value: "bottom", label: "Bottom" },
];

/**
 * Reveal the top N cards ("reveal until…", impulse effects, big flips):
 * like scry, but each card can go to hand / battlefield / graveyard / exile /
 * top (ordered) / bottom — with an option to shuffle the bottomed cards first.
 */
export function RevealXModal({ count }: { count: number }) {
  const g = useGameStore();
  const closeModal = useUiStore((s) => s.closeModal);

  const topIds = useMemo(
    () => (g.zoneOrder[PLAYER_ID]?.library ?? []).slice(0, count),
    // capture once on open — the library doesn't change while the modal is up
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [],
  );
  const [dest, setDest] = useState<Record<string, Dest>>(
    Object.fromEntries(topIds.map((id) => [id, "top" as Dest])),
  );
  const [topOrder, setTopOrder] = useState<string[]>(topIds);
  const [shuffleBottom, setShuffleBottom] = useState(true);

  const setCardDest = (id: string, d: Dest) => {
    setDest((prev) => ({ ...prev, [id]: d }));
    setTopOrder((prev) => {
      const without = prev.filter((x) => x !== id);
      return d === "top" ? [...without, id] : without;
    });
  };

  const moveInTop = (id: string, dir: -1 | 1) => {
    setTopOrder((prev) => {
      const idx = prev.indexOf(id);
      const next = idx + dir;
      if (idx < 0 || next < 0 || next >= prev.length) return prev;
      const arr = [...prev];
      [arr[idx], arr[next]] = [arr[next]!, arr[idx]!];
      return arr;
    });
  };

  const byDest = (d: Dest) => topIds.filter((id) => dest[id] === d);

  const confirm = () => {
    // Zone moves first (each id leaves the library), then the top/bottom
    // ordering resolves through the same path scry uses.
    for (const zone of ["hand", "battlefield", "exile"] as const) {
      const ids = byDest(zone);
      if (ids.length > 0) g.moveCards(ids, zone);
    }
    let bottomIds = byDest("bottom");
    if (shuffleBottom && bottomIds.length > 1) {
      bottomIds = [...bottomIds];
      for (let i = bottomIds.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [bottomIds[i], bottomIds[j]] = [bottomIds[j]!, bottomIds[i]!];
      }
    }
    g.resolveTopCards(topOrder, bottomIds, byDest("graveyard"));
    closeModal();
  };

  return (
    <Modal title={`Reveal ${count}`} wide>
      <p className="mb-3 text-[11px] text-stone-500">
        Send each card where the effect says. Cards staying on top resolve in the order shown
        (first = topmost) — use ◀ ▶ to reorder.
      </p>
      <div className="flex flex-wrap gap-3">
        {topIds.map((id) => {
          const inst = g.instances[id];
          if (!inst) return null;
          const card = g.cards[inst.cardId];
          const d = dest[id] ?? "top";
          const topIdx = topOrder.indexOf(id);
          return (
            <div key={id} className="flex w-32 flex-col gap-1.5">
              <div className="relative">
                <CardImage card={card} flipped={inst.flipped} className="aspect-[5/7] w-full" />
                {d === "top" && (
                  <span className="absolute -top-1.5 -left-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-emerald-600 text-[10px] font-bold text-white">
                    {topIdx + 1}
                  </span>
                )}
              </div>
              {d === "top" && (
                <div className="flex justify-center gap-1">
                  <button onClick={() => moveInTop(id, -1)} className="rounded bg-stone-800 px-2 text-xs text-stone-300 hover:bg-stone-700">◀</button>
                  <button onClick={() => moveInTop(id, 1)} className="rounded bg-stone-800 px-2 text-xs text-stone-300 hover:bg-stone-700">▶</button>
                </div>
              )}
              <div className="grid grid-cols-2 gap-1">
                {DESTS.map((opt) => (
                  <button
                    key={opt.value}
                    onClick={() => setCardDest(id, opt.value)}
                    className={`rounded px-1.5 py-1 text-[10px] font-semibold transition ${
                      d === opt.value
                        ? "bg-emerald-700 text-white"
                        : "bg-stone-800 text-stone-400 hover:bg-stone-700"
                    }`}
                  >
                    {opt.label}
                  </button>
                ))}
              </div>
            </div>
          );
        })}
      </div>
      <div className="mt-4 flex items-center justify-between gap-3">
        <label className="flex cursor-pointer items-center gap-2 text-[11px] text-stone-400">
          <input
            type="checkbox"
            checked={shuffleBottom}
            onChange={(e) => setShuffleBottom(e.target.checked)}
            className="accent-emerald-600"
          />
          Shuffle bottomed cards before placing them
        </label>
        <button
          onClick={confirm}
          className="rounded-md bg-emerald-700 px-4 py-1.5 text-xs font-bold text-white hover:bg-emerald-600"
        >
          Confirm
        </button>
      </div>
    </Modal>
  );
}
